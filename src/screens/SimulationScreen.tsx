...
export default function SimulationScreen() {
  const { molecule, admin, setSimulationResult, setScreen, simulationResult, moleculeAnalysis } = useExperience();
  ...
  }, [totalDuration, effectiveDuration, breakProgressTarget, admin.simulationIntensity, fittedAtoms, simulationResult, moleculeAnalysis]);
              ...
              <div className="glass-card rounded-2xl px-6 py-4 text-sm text-left space-y-2 min-w-[260px] max-w-sm w-full">
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-1">
                  {moleculeAnalysis.formula && (
                    <p className="text-muted-foreground">
                      <span className="text-foreground font-medium">Fórmula:</span>{' '}
                      {moleculeAnalysis.formula}
                    </p>
                  )}
                  {moleculeAnalysis.molecular_weight > 0 && (
                    <p className="text-muted-foreground">
                      <span className="text-foreground font-medium">Massa:</span>{' '}
                ...
                {isBreak && moleculeAnalysis.break_temperature != null && (
                  <div className="border-t border-border/40 pt-2 mt-1">
                    <p className="text-muted-foreground">
                      <span className="text-foreground font-medium">Temperatura de quebra:</span>{' '}
                      <span className="text-destructive font-bold">
                        {Math.round(moleculeAnalysis.break_temperature).toLocaleString('pt-BR')} K
                      </span>
                    </p>
                  </div>
                )}
